...
// Téléchargement URL -> Buffer (redirections + timeout)
function fetchUrlToBuffer(url) {
  return new Promise((resolve, reject) => {
    ...
    req.setTimeout(15000, () => {
      req.destroy(new Error("Timeout téléchargement image"));
    });
  });
}
...
// Validation légère: taille mini ; on n’interdit pas si magic bytes inconnus
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (Array.isArray(content.sections)) {
        // Pour utiliser await dans forEach → on passe en for...of
        (async () => {
            ...
            if (section.imageUrl || section.imagePath || section.image) {
              try {
                ...
                try {
                  ...
                  // JPEG exotiques → réessayer avec width seulement
...
// Echo (utilisé par ton Action pour tester le parsing JSON)
