...
// ========================= ROUTE: Envoyer email avec base64 CHUNKED =========================
app.post('/send-email-base64', async (req, res) => {
  try {
    const { to, subject, message, image, imageChunks, imageName, chunkIndex, totalChunks } = req.body;

    console.log('========================================');
    console.log('📧 NOUVELLE REQUÊTE: /send-email-base64');
    console.log('========================================');

    // ========== CAS 1: Image en un seul chunk (ancienne méthode) ==========
    if (image && !imageChunks) {
      console.log('📦 Mode: Single chunk (complet)');
      console.log('Destinataire:', to);
      console.log('Base64 reçu - Longueur:', image.length, 'caractères');
    ...
    // ========== CAS 2: Image en chunks multiples (nouvelle méthode) ==========
    if (imageChunks) {
      console.log(`📦 Mode: Chunks (${chunkIndex + 1}/${totalChunks})`);
      
      // Initialiser le stockage des chunks en mémoire (par session)
      if (!app.locals.uploadSessions) {
        app.locals.uploadSessions = {};
      }

      const sessionId = `${to}_${imageName}_${Date.now()}`;
      
      if (!app.locals.uploadSessions[sessionId]) {
        app.locals.uploadSessions[sessionId] = {
          chunks: [],
          to: to,
          subject: subject,
          message: message,
      ...
      const session = app.locals.uploadSessions[sessionId];
      session.chunks[chunkIndex] = imageChunks;

      console.log(`✅ Chunk ${chunkIndex + 1}/${totalChunks} reçu`);

      // Vérifier si tous les chunks sont reçus
      const allChunksReceived = session.chunks.length === totalChunks && 
                                session.chunks.every(chunk => chunk);

      if (allChunksReceived) {
        console.log('✅ TOUS les chunks reçus! Reconstruction en cours...');
        
        // Recombiner les chunks
        const completeBase64 = session.chunks.join('');
        console.log(`📊 Base64 complet: ${completeBase64.length} caractères`);
        ...
        try {
          ...
          // Nettoyer la session
          delete app.locals.uploadSessions[sessionId];
