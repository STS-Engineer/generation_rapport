...
function safeAddImage(doc, { caption, imageBase64 }, opts = {}) {
  ...
  try {
    if (doc.y > 600) doc.addPage();
    if (caption) {
      doc.fontSize(12).font("Helvetica-Oblique").fillColor("#6b7280").text(caption, { align: "center" });
      doc.moveDown(0.5);
    }
...
function generatePDF(content) {
  return new Promise((resolve, reject) => {
    try {
      ...
      // Sections (AVEC SUPPORT D'IMAGES)
      if (Array.isArray(content.sections)) {
        content.sections.forEach((section, index) => {
          ...
          // Image de la section (si présente)
          if (section.imageBase64) {
            safeAddImage(
              doc, 
              { 
                caption: section.imageCaption || `Image - ${section.title}`, 
                imageBase64: section.imageBase64 
              }, 
              { label: `section-${index}`, maxH: 280 }
            );
          }
...
app.post("/api/generate-and-send", async (req, res) => {
  try {
    ...
    // Validation des sections avec images
    for (let i = 0; i < rc.sections.length; i++) {
      const section = rc.sections[i];
      if (!section.title || !section.content) {
        return res.status(400).json({ 
          success: false, 
          error: `Section ${i + 1} invalide : title et content requis` 
        });
      }
      
      // Validation optionnelle de l'image
      if (section.imageBase64) {
        const imgBuf = b64ToBufferMaybe(section.imageBase64);
        if (!looksLikePngOrJpeg(imgBuf)) {
          return res.status(400).json({ 
            success: false, 
            error: `Image de la section "${section.title}" invalide : doit être PNG ou JPEG valide en base64` 
          });
        }
      }
    }

    // Validation du tableau (si présent)
    if (rc.table) {
    ...
    // Validation du graphe (si présent)
